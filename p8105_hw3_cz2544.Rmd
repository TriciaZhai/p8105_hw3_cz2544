...
=======
  distinct(year, locationabbr, mean) %>% #filter(locationabbr == "WV") %>% View()
>>>>>>> aee9cfe6fb10990c91b4e2b8d57a9e5133b6f881
  ...
  facet_grid(~response) 
<<<<<<< HEAD
p2 + guides(col = guide_legend(ncol = 26, nrow = 2, override.aes = list(legend,size = 1.5)))
=======
p2 + guides(col = guide_legend(ncol = 26, nrow = 2, override.aes = list(size = 1.5)))
>>>>>>> aee9cfe6fb10990c91b4e2b8d57a9e5133b6f881
```
...
**Description**The dataset has 15 variables and 1384617 observations without missing values. The names have been cleaned with character variables of aisle, department, eval_set and product_name, integer variables of add_to_cart_order, aisle_id, days_since_prior_order, department_id, order_dow, order_hour_of_day, order_id, order_number, product_id, reordered and user_id. The variables can be put into 6 classes: 1.id to track a order/orderer (order, user)(id), 2. order time/location distribution (order_hour_of_day, order_number), 3. catergories and identification of a product (department, aisle, product)(name, id), 4. frequency of order (reorder, days_since_prior_order), 5. other character of user behavior (add_to_cart_order). And two variables I don not know what exactly meaning is (order_dow, , order_number).

It seems to be a dataset about shopping records of `r n_distinct(instacart$user_id)` users and their `r n_distinct(instacart$order_id)` orders, thus each user had made only one order during the dataset study period of one day through 0 to 23, most often between`r quantile(instacart$order_hour_of_day,c(0.25, 0.75))`,  all happened in eval_set of train. There are a total `r n_distinct(instacart$product_id)` kinds of products, in `r n_distinct(instacart$department)` departments and `r n_distinct(instacart$aisle)` aisles. Products, aisles, and departments were recored with both name(character) and ID(integer). 
...
The mean number of order_dow is `r mean(instacart$order_dow)`+/-`r sd(instacart$order_dow)`, and median is `r median(instacart$order_dow)`, range between `r quantile(instacart$order_dow, c(0,1))`. The mean number of order_number is `r mean(instacart$order_number)`+/-`r sd(instacart$order_number)`, and median is `r median(instacart$order_number)`, range between `r quantile(instacart$order_number, c(0,1))`.
...
```{r explore_popularity_diatribution, eval = FALSE}
p3 = instacart %>% 
  group_by(department) %>% 
  ...
  theme(legend.direction = "horizontal", legend.key.size = unit(0.4, "cm"),
        legend.text = element_text(size = 7.5),
        legend.title = element_text(size = 8))
p3 + guides(col = guide_legend(ncol = 11, nrow = 2, override.aes = list(size = 1.5)))
```
...
aisle_rank =
  ...
  summarise(n= n()) %>%
  arrange((n)) %>% 
  mutate(rank = row_number(), aisle = factor(aisle, levels = c(aisle)) )

p4 = aisle_rank %>% 
  ggplot(aes(x = rank, y = n, color = aisle)) +
  geom_point(size = 2) +
  scale_y_log10() +
  scale_x_discrete() +
  labs(
  ...
  viridis::scale_color_viridis( name = "aisle", discrete = TRUE) +
  theme(legend.direction = "horizontal", legend.key.size = unit(2, "mm"),
        legend.title = element_text(size = 3.5))
  
p4 + guides(col = guide_legend(label.theme = element_text(angle = -90, size = 4), label.position = "bottom",
                               ncol = 134, nrow = 1, override.aes = list(legend.position = "bottom", size = 2)))
```
**Comment:**The range of number of items ordered in each aisle is very big, thus display the number in log transformation might make it easier to read.
