...
p4 = aisle_rank %>% 
  ...
  theme(legend.direction = "horizontal", legend.title =  NULL)
  
p4_s = p4 + guides(size = "none", title = element_text(size = 4),
                   axis.title.x = element_text(size = 3), 
                   axis.title.y = element_text(size = 3), axis.text.y = element_text(size = 2),
                   col = guide_legend(label.theme = element_text(angle = -90, size = 2), 
                               label.position = "bottom", label.vjust = 0, title = NULL,
                               ncol = 134, nrow = 1, keywidth = unit(0.12, "mm"),
                               override.aes = list(legend.position = "bottom", size = 0.8)))
...
instacart %>% 
  ...
  summarise(n=n()) %>%  
  arrange(desc(n)) %>% View()
  distinct(aisle, product_name, n) %>% 
  spread(key = aisle, value = c("baking ingredients", "dog food care", "packaged vegetables fruits")) %>% View()
  head() %>%
  knitr::kable(digits = 2)
```
